...
function enviarCodigoVerificacao(string $toEmail, string $toName, string $codigo): array {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Mecatec: Seu Codigo de Cadastro';
        $mail->Body = '<div style="background:#f6f7fb;padding:32px 20px;border-radius:12px;color:#222;font-family:Poppins,sans-serif;max-width:480px;margin:auto;box-shadow:0 0 16px #00000022">
            ...
            <p style="margin:8px 0 24px 0">Para ativar sua conta na Mecatec, utilize o código de verificação abaixo:</p>
            ...
            <p style="margin-top:24px;font-size:.95rem;color:#666">Insira este código na tela de confirmação. Se você não solicitou este cadastro, ignore este e-mail.</p>
        ...
        $mail->send();
        return ['success'=>true, 'message'=>'E-mail de verificação enviado com sucesso para '.$toEmail];
    } catch (Exception $e) {
